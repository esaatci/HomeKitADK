...
int32_t getState(void) {
    return accessoryConfiguration.state.lightBulbColor;
}
...
HAP_RESULT_USE_CHECK
HAPError HandleLightBulbColorRead(
                HAPAccessoryServerRef* server HAP_UNUSED,
                const HAPUInt32CharacteristicReadRequest* request HAP_UNUSED,
                uint32_t* value,
                void* _Nullable context HAP_UNUSED) {
    *value = accessoryConfiguration.state.lightBulbColor;
    HAPLogInfo(&kHAPLog_Default, "%s: %d", __func__, *value);
    return kHAPError_None;
}

HAP_RESULT_USE_CHECK
HAPError HandleLightBulbColorWrite(
                HAPAccessoryServerRef* server,
                const HAPUInt32CharacteristicWriteRequest* request,
                uint32_t value,
                void* _Nullable context HAP_UNUSED) {

    accessoryConfiguration.state.lightBulbColor = value;
...
static void SetInitialAcccessoryState(void) {
    accessoryConfiguration.state.lightBulbOn = false;
    accessoryConfiguration.state.lightBulbColor = 50;
    accessoryConfiguration.state.lightBulbBrightness = 100;
    SaveAccessoryState();
}
